...
public class TeacherDaoImpl implements AutoCloseable, TeacherDao {
    ...
    @Override
    public void add(Teacher teacher, int password) throws DaoException {
        try (PreparedStatement statement1 = connection.prepareStatement(SQL_INSERT_USER, Statement.RETURN_GENERATED_KEYS);
             PreparedStatement statement2 = connection.prepareStatement(SQL_INSERT_TEACHER, Statement.RETURN_GENERATED_KEYS);
             PreparedStatement statement3 = connection.prepareStatement(SQL_INSERT_TEACHER_SUBJECT)) {
            statement1.setString(1, teacher.getEmail());
            statement1.setInt(2, password);
            statement1.setString(3, teacher.getFirstName());
            statement1.setString(4, teacher.getLastName());
            statement1.executeUpdate();
            ResultSet resultSet = statement1.getGeneratedKeys();
            if (resultSet.next()) {
                teacher.setId(resultSet.getLong(1));
            }
            statement2.setLong(1, teacher.getId());
            statement2.executeUpdate();
            resultSet = statement2.getGeneratedKeys();
            if (resultSet.next()) {
                teacher.setTeacherId(resultSet.getLong(1));
            }
            statement3.setLong(1, teacher.getTeacherId());
            for (int i = 0; i < teacher.getSubjectList().size(); i += 1) {
                statement3.setLong(2, teacher.getSubjectList()
                                                          .get(i)
                                                          .getSubjectId());
